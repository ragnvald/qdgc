	...
	if purge=1 then
   	...
   	for counter in 1..qdgc_level loop
		...
		with grid as (
			...
			select qdgc_getqdgc(ST_X(ST_Centroid(geom)),ST_Y(ST_Centroid(geom)),counter),area_reference, counter,(1/(2^counter)), ST_X(ST_Centroid(geom)),ST_Y(ST_Centroid(geom)),(st_area(st_transform(geom, 9822))/1000000), geom from grid;
